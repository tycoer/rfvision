...
model = dict(
    ...
    nocs_head=dict(
        ...
        n_parts=3,
        mixed_pred=True,
        loss_miou=dict(type='Arti_MIoU_Loss'),
        loss_nocs=dict(type='Arti_NOCS_Loss', TYPE_L='L2', MULTI_HEAD=True, SELF_SU=False),
        loss_vect=dict(type='Arti_VECT_Loss', TYPE_L='L2', MULTI_HEAD=False, SELF_SU=False),
...
data = dict(
    ...
    train=dict(
        type=[dataset_type] * 2,
        ann_file=[data_root + 'scissor/train.txt',
                  data_root + 'scissor_v2/train_meta.txt'],
        img_prefix=[data_root + 'scissor/',
                    data_root + 'scissor_v2/'],

        intrinsics_path=data_root + 'scissor/camera_intrinsic.json',
...
log_level = 'INFO'
work_dir = './work_dirs/ancsh_pointnet_1x_{}_{}2_v2'.format(data_track, category)
load_from = None
resume_from = None
workflow = [('train', 1)]
