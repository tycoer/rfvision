    ...
    while head < num_samples:
        ...
        if atc_vec is not None:
            samples[head : min(head + max_batch, num_samples), 3] = (
                decode_sdf(decoder, lat_vec, sample_subset, atc_vec=atc_vec, do_sup_with_part=do_sup_with_part, specs=specs)
                .squeeze(1)
                .detach()
                .cpu()
            )
            ...
            samples[head : min(head + max_batch, num_samples), 3] = (
                decode_sdf(decoder, lat_vec, sample_subset, specs=specs)
                .squeeze(1)
                .detach()
                .cpu()
            )
