...
from models.ASDF.utils.metrics import compute_trimesh_chamfer
    ...
    for dataset in split:
        for class_name in split[dataset]:
            if mode=='recon_testset':
                ...
                all_names = sorted(glob.glob(os.path.join( experiment_directory, 'Results_inter_testset', checkpoint, 'Meshes', dataset, '*.ply' )))
            elif mode=='inter_testset_ttt':
                all_names = sorted(glob.glob(os.path.join( experiment_directory, 'Results_inter_testset_ttt', checkpoint, 'Meshes', dataset, '*.ply' )))
            ...
            for instance_name in all_names:
                ...
                if class_name=='laptop' or class_name=='stapler' or class_name=='washing_machine' or class_name=='oven' or class_name=='door':
                    instance_name = reconstructed_name[:7] + reconstructed_name[-8:-4]
                elif class_name=='eyeglasses' or class_name=='refrigerator':
                    instance_name = reconstructed_name[:7] + reconstructed_name[-12:-4]
                else:
