...
def run_simulation(
    ...
    # add hand
    base_tmp_dir = "/tmp/CPF_dev/simulation/objs"
    ...
    if use_vhacd:
        ...
        obj_collision_id = p.createCollisionShape(p.GEOM_MESH, fileName=obj_tmp_fname, physicsClientId=conn_id)
        if verbose:
            time2 = time.time()
            print("Computed v-hacd decomposition at res {} {:.6f} s".format(vhacd_resolution, (time2 - time1)))
    else:
        obj_collision_id = p.createCollisionShape(p.GEOM_MESH, vertices=obj_verts, physicsClientId=conn_id)
...
def process_sample(
    ...
    if sample_idx % sample_vis_freq == 0:
        save_video = True
        save_video_path = os.path.join(save_gif_folder, "{:04d}.gif".format(sample_idx))
        save_obj_path = os.path.join(save_obj_folder, "{:04d}_obj.obj".format(sample_idx))
        save_hand_path = os.path.join(save_obj_folder, "{:04d}_hand.obj".format(sample_idx))
        if save_all_steps:
            save_obj_steps_folder = os.path.join(save_obj_folder, "{:04d}_obj".format(sample_idx))
            save_hand_steps_folder = os.path.join(save_obj_folder, "{:04d}_hand".format(sample_idx))
