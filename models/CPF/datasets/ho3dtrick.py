...
class HO3DSyntTrick(ho3dsynt.HO3DSynt):
    def __init__(
        self,
        data_root="data",
        data_split="train",
        ...
        **kwargs,
        ...
        self.name = "HO3D_synthesis_trick"
    ...
    def load_dataset(self):
        ...
        for i in range(len(self)):
            self.cam_intr.append(self._load_cam_intr(i))
        # for i in range(len(self)):
    ...
    def load_contact_infos(self, idxs, seq_map):
        ...
        for i in range(len(idxs)):
            ...
            path = path.replace("_synthesis_trick", "")  # load no synt contact info
    ...
    def _load_cam_intr(self, idx):
        path = self.get_image_path(idx)
        path = path.replace("rgb", "meta")
        path = path.replace(".png", "_info.pkl")
        annot = pickle.load(open(path, "rb"))
        return annot["ci"].astype(np.float32)

    def get_cam_intr(self, idx):
        return self.cam_intr[idx]
...
def main(args):
    ...
    #     block_rot=True,
    # )
    # from hocontact.datasets.ho3d import HO3D
    ...
    #     if os.path.isfile(ho_dataset.get_image_path(i).replace("rgb", "meta").replace("png", "npy")):
    #         continue
    #     raw_joints = (cam_extr @ raw_dataset.get_joints3d(i // 8).T).T
    #     assert np.allclose(raw_dataset.get_joints3d(i // 8), HO3D.get_joints3d(ho_dataset, i))
    #     target_joints = (cam_extr @ ho_dataset.get_joints3d(i).T).T
    #     res = fit_transf(raw_joints, target_joints)
    #     raw_annot = raw_dataset.get_annot(i // 8)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--synt_factor", type=int, choices=[1, 2, 3, 4, 5, 6, 7, 8], default=1)
