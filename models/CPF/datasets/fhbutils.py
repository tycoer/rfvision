...
def load_objects_full_color(obj_root="data/fhbhands_supp/Object_models", object_names=["juice"]):
    all_models = OrderedDict()
    for obj_name in object_names:
        obj_path = os.path.join(obj_root, "{}_model".format(obj_name), "{}_color.obj".format(obj_name))
        mesh = trimesh.load(obj_path, process=False)
        if obj_name == "juice":
            obj_name = "juice_bottle"
        all_models[obj_name] = {
...
def get_skeletons(skeleton_root, subjects_info, use_cache=True):
    ...
    if os.path.exists(cache_path) and use_cache:
        ...
        print("Loaded fhb skel info from {}".format(cache_path), "yellow")
    else:
