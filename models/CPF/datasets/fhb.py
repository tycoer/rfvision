...
import numpy as np
import torch
from . import fhbutils
from .hodata import HOdata
from .hoquery import BaseQueries, ContactQueries, get_trans_queries
from ..utils import meshutils
from ..utils.contactutils import process_contact_info
from ..utils.visualize.vis_contact_info import create_vertex_color, view_vertex_contact
...
class FPHB(hodata.HOdata):
    def __init__(
        ...
        **kwargs,
        ...
        if self.enable_contact:
            self.all_queries.update(
                {
                    ContactQueries.HAND_PALM_VERT_IDX,
    ...
    def _preload(self):
        ...
        # todo: this points to wrong path!
        # todo: for synthesize fhb, need to store its root, its origin_root and its origin_supp
        self.root_supp = os.path.join(self.data_root, f"{self.name}_supp")
        self.root_extra_info = self.assets_root
    ...
    def load_dataset(self):
        ...
        if os.path.exists(self.cache_path) and self.use_cache:
            ...
            self.fhb_objects = fhbutils.load_objects(
                obj_root=os.path.join(self.root_supp.replace("_synthesis", ""), "Object_models"),
            ...
            if self.name == "fhbhands" and self.for_render:
                self.obj_meshes_full_color = fhbutils.load_objects_full_color(
                    obj_root=os.path.join(self.root_supp.replace("_synthesis", ""), "Object_models"),
                    object_names=all_objects,
                )

            obj_infos = fhbutils.load_object_infos(
                os.path.join(self.root.replace("_synthesis", ""), "Object_6D_pose_annotation_v1_1")
            )
            if self.enable_contact:
                contact_infos = fhbutils.load_contact_infos(
                    os.path.join(self.root_supp.replace("_synthesis", ""), "Object_contact_region_annotation_v512")
                )
            ...
            if self.split_mode == "actions":
                ...
                if self.data_split == "train":
                    ...
                    print(f"Split {self.data_split} not in [train|test] for split_type subjects")
                ...
                error = print(f"split_type should be in [action|objects|subjects], got {self.split_mode}")
            ...
            for subject, action_name, seq_idx, frame_idx in sample_list:
                ...
                if self.enable_contact:
                    # ====== CONTACT >>>>>>>
                    contacts_info_pkl_path = contact_infos[subject][(action_name, seq_idx, frame_idx)]
                    # <<<<<<<<<<<<<<<<<<<<<<<
                ...
                if self.enable_contact:
                    contact_info_targets.append(contacts_info_pkl_path)

            # assemble annotation
            mano_objs, mano_infos = fhbutils.load_manofits(sample_infos, fit_root=f'{self.root_extra_info}/fhbhands_fits')
            annotations = {
            ...
            if self.enable_contact:
                annotations["contact_infos"] = contact_info_targets

            if self.rand_size != 0:  # Become effective ONLY in fhb's subclass: eg. fhbsynt
                print("FPHB subclass is duplicating annot")
        ...
        if self.enable_contact:
            self.contact_infos = annotations["contact_infos"]
        ...
        if self.load_objects_voxel:
            self.fhb_objects_voxel = fhbutils.load_objects_voxel(
                obj_root=os.path.join(self.root_supp.replace("_synthesis", ""), "Object_models_binvox"),
                object_names=all_objects,
            )
        ...
        print(f"Got {len(self.image_names)} samples for data_split {self.data_split}")
        return
    ...
    def get_center_scale(self, idx):
        if self.full_image:
            ...
            print("Non full_image mode is not implements")
    ...
    def get_processed_contact_info(self, idx):
        ...
        if "fhbhands" in img_path:
            if "pour_milk" in img_path:
                contact_info_pkl_path = re.sub(r"Subject_\d", "Subject_1", contact_info_pkl_path)
                contact_info_pkl_path = re.sub(r"/\d/", "/2/", contact_info_pkl_path)
                contact_info_pkl_path = re.sub(r"\d{4}.pkl", "0159.pkl", contact_info_pkl_path)
            elif "/Subject_6/pour_liquid_soap" in img_path:
                contact_info_pkl_path = re.sub(r"Subject_\d", "Subject_3", contact_info_pkl_path)
                contact_info_pkl_path = re.sub(r"\d{4}.pkl", "0010.pkl", contact_info_pkl_path)
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
