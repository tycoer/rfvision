...
#         self.contact_sym = contact_sym
#         self.contact_zones = contact_zones

#         self.contact_lambda = contact_lambda
#         self.collision_lambda = collision_lambda

#     def compute_loss(self, results, sample):
#         losses = dict()
#         target_device = results["recov_hand_verts3d"].device
#         batch_size = results["recov_hand_verts3d"].shape[0]

#         total_attr_loss = torch.Tensor([0.0]).to(target_device)
#         total_penetr_loss = torch.Tensor([0.0]).to(target_device)

#         for sample_id in range(batch_size):

#             # get true faces
#             tmp_faces = sample[BaseQueries.OBJ_FACES][sample_id].long().to(target_device)
#             tmp_faces = tmp_faces[sample[CollateQueries.FACE_PADDING_MASK][sample_id].bool()]

#             (attr_loss, penetr_loss, _, _,) = compute_contact_loss(
#                 results["recov_hand_verts3d"][sample_id : sample_id + 1],
#                 sample[BaseQueries.HAND_FACES][sample_id : sample_id + 1].long().to(target_device),
#                 results["recov_obj_verts3d"][sample_id : sample_id + 1],
#                 tmp_faces,
...
#                 contact_sym=self.contact_sym,
#                 contact_zones=self.contact_zones,
#             )
#             total_attr_loss += attr_loss
#             total_penetr_loss += penetr_loss
#         total_attr_loss /= batch_size
#         total_penetr_loss /= batch_size

#         contact_loss = self.contact_lambda * total_attr_loss + self.collision_lambda * total_penetr_loss
#         losses["penetration_loss"] = total_penetr_loss
#         losses["attraction_loss"] = total_attr_loss
#         losses["contact_loss"] = contact_loss
#         return contact_loss, losses
