...
from models.CPF.datasets.cionline import CIOnline
from models.CPF.datasets.cionline import CIAdaptQueries, CIDumpedQueries
from ..postprocess.hand_optimizer import HandOptimizer
from ..utils.collisionutils import (
    penetration_loss_hand_in_obj,
    solid_intersection_volume,
)
from ..utils.disjointnessutils import region_disjointness_metric
from ..utils.kutils import KMetric
...
def run_sample_by_idx(
    ...
    hand_closed_path="data/info/closed_hand/hand_mesh_close.obj",
):
    ...
    hand_joints_dist_before = torch.mean(
        torch.norm(
            torch.from_numpy(hand_joints_np).float() - torch.from_numpy(hand_joints_adapt_np).float(), p=2, dim=1,
        )
    ).item()
    ...
    object_dist_after = (
        ...
        if cidata.hodataset.split_mode == "official"
        else torch.mean(torch.norm(torch.from_numpy(obj_verts_3d_adapt_np).float() - obj_verts_pred, p=2, dim=1,)).item()
    )
...
def worker(
    ...
    cprint(
        f"====== {worker_id:>3} begin: {begin_index:0>4} end: {end_index:0>4} len: {len(ci_online)} >>>>>>", "cyan",
    )
    cprint(
        f"====== {worker_id:>3} using device: {device} >>>>>>", "cyan",
    )
    hoptim = HandOptimizer(
        device,
        lr=lr,
        n_iter=n_iter,
        verbose=False,
    ...
    kmetric = KMetric()
    cprint(
        f"====== optimizer created on device: {device} >>>>>>", "cyan",
    )
    ...
    for i in range(begin_index, end_index):
        try:
            ...
            color_str = better_color if res["penetration_depth_after"] < res["penetration_depth_before"] else worse_color
            ...
            color_str = (
                better_color if res["disjointness_tip_only_after"] < res["disjointness_tip_only_before"] else worse_color
            )
