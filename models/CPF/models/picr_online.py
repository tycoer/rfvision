...
from models.CPF.datasets.hoquery import TransQueries, ContactQueries, one_query_in
from .hourglass import StackedHourglass
from .contacthead import PointNetContactHead
from .honet import HONet
from ..utils.netutils import freeze_batchnorm_stats
from manopth.rodrigues_layer import batch_rodrigues
from models.CPF.utils.ioutils import reload_checkpoint
...
class PicrHourglassPointNet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.init_honet_ckpt = init_honet_ckpt # tycoer
        self.init_picr_ckpt = init_picr_ckpt # tycoer
        ...
        self.base_net = StackedHourglass(hg_stacks, hg_blocks, hg_classes)
        ...
        if self.init_honet_ckpt is not None:
            reload_checkpoint(self.ho_net, resume_path=self.init_honet_ckpt, as_parallel=False, map_location=None)
        if self.init_picr_ckpt is not None:
            reload_checkpoint(self.base_net, resume_path=self.init_honet_ckpt, as_parallel=False, map_location=None, startswith='base_net')
            reload_checkpoint(self.contact_head, resume_path=self.init_honet_ckpt, as_parallel=False, map_location=None, startswith='contact_head')
    ...
    def forward(self, sample):
        ...
        self.ho_net.eval()
        with torch.no_grad():
            honet_results = self.ho_net(sample)
        ...
        has_contact_supv = one_query_in(sample.keys(), [ContactQueries.VERTEX_CONTACT,])
