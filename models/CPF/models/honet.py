...
from models.CPF.utils import handutils
# from hocontact.models.bases import resnet
from .transhead import TransHead, recover_3d_proj
# from hocontact.models import transhead
from ..utils import netutils
from .manobranch import ManoBranch
# from hocontact.utils.logger import logger
from ..datasets.hoquery import TransQueries, BaseQueries, one_query_in
...
class HONet(nn.Module):
    def __init__(
        ...
        if int(resnet_version) == 18:
            ...
            base_net = ResNet(pretrained=f'torchvision://resnet{resnet_version}', depth=resnet_version)
        elif int(resnet_version) == 50:
            img_feature_size = 2048
            base_net = ResNet(pretrained=f'torchvision://resnet{resnet_version}', depth=resnet_version)
        else:
            # print("Resnet {} not supported".format(resnet_version))
        ...
        if self.mano_fhb_hand:
            load_fhb_path = f"{assets_root}/mano/fhb_skel_centeridx{mano_center_idx}.pkl"
    ...
    def recover_mano(self, sample, features):
        # Get hand projection, centered
        ...
        if self.mano_lambda_recov_joints3d or self.mano_lambda_recov_verts3d:
            ...
            camintr = sample[TransQueries.CAM_INTR]
            recov_joints3d, hand_center3d = recover_3d_proj(
                mano_results["joints3d"], camintr, final_scale, final_trans, input_res=(width, height)
            )
            recov_hand_verts3d = mano_results["verts3d"] + hand_center3d
    ...
    def recover_object(self, sample, features):
        """
        ...
        can_obj_verts = sample[BaseQueries.OBJ_CAN_VERTS]
        ...
        if BaseQueries.OBJ_CORNERS_3D in sample:
            can_obj_corners = sample[BaseQueries.OBJ_CAN_CORNERS]
    ...
    def forward(self, sample):
        ...
        features = self.base_net(image)[-1]
        features = features.mean(3).mean(2).view(features.size(0), -1) # tycoer
        ...
        if has_mano_supv and self.mano_lambdas:
            mano_results = self.recover_mano(sample, features)
            results.update(mano_results)

        if has_obj_supv and self.obj_lambdas:
            obj_results = self.recover_object(sample, features)
