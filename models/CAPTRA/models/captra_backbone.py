...
from ..utils.blocks import get_point_mlp, RotationRegressor
from ..utils.part_dof_utils import merge_reenact_canon_part_pose, convert_pred_rtvec_to_matrix
from ..utils.procrustes import scale_pts_mask, translate_pts_mask, transform_pts_2d_mask, rot_around_yaxis_to_3d
from ..utils.pose_fit import part_fit_st_no_ransac
from .captra_pointnet2 import CaptraCoordNetPointNet2, CaptraRotNetPointNet2
...
class CoordNet(nn.Module):
    def __init__(self,
        ...
        self.backbone = CaptraCoordNetPointNet2()
        in_dim = backbone_out_dim
        num_parts = obj_cfg['num_parts']
        self.num_parts = num_parts
        self.sym = obj_cfg['sym']
        seg_dim = self.num_parts + extra_dims
...
class RotationRegressionBackbone(nn.Module):
    def __init__(self,
        ...
        self.num_parts = obj_cfg['num_parts']
        self.encoder = CaptraRotNetPointNet2()
        self.sym = obj_cfg['sym']
        self.pose_pred = RotationRegressor(backbone_out_dim, self.num_parts,
                                           symmetric=self.sym)
...
class PartCanonNet(nn.Module):
    def __init__(self,
        ...
        self.regress_net = RotationRegressionBackbone(obj_cfg, backbone_out_dim)
        self.num_parts = obj_cfg['num_parts']
        self.sym = obj_cfg['sym']
    ...
    def forward(self, input, test_mode=False):
        ...
        """
        self.device = input['points'].device
        state = input['state']
