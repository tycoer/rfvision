...
from models.CAPTRA.utils.vis_utils import plot3d_pts
from mmcv.ops.furthest_point_sample import furthest_point_sample
from models.CAPTRA.utils.utils import ensure_dir, ensure_dirs
...
def farthest_point_sample(xyz, npoint, device):
    ...
    if torch.cuda.is_available():
        if len(xyz) > 5 * npoint:
            ...
            torch_idx = furthest_point_sample(torch_xyz.cuda(), npoint)
            torch_idx = torch_idx.cpu().numpy().reshape(-1)
            idx = idx[torch_idx]
            return idx
        else:
            torch_xyz = torch.tensor(xyz).cuda().reshape(1, -1, 3).float().contiguous()
            torch_idx = furthest_point_sample(torch_xyz, npoint)
...
def get_pts_for_bbox_overlay(root_dset, obj_category, instance, track_num, frame_i, num_parts, num_points=4096,
    ...
    from models.CAPTRA.utils.arti_data_process import read_cloud
